...
func main() {
  ...
  authService := auth.NewService(db, rdb, mailer, cfg)

  if err := ensureDirs(cfg); err != nil {
    log.Fatalf("初始化存储目录失败: %v", err)
  }

  router := httpserver.NewRouter(cfg, authService)

  workerService := &worker.Worker{Store: db, Redis: rdb, Config: cfg}
  workerCtx, workerCancel := context.WithCancel(context.Background())
  go workerService.Start(workerCtx)
