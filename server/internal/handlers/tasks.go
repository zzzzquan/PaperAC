...
func (h *TaskHandler) CreateTask(c *gin.Context) {
  ...
  if err := h.Redis.PushTask(c.Request.Context(), h.Config.TaskQueueName, taskID.String()); err != nil {
    _ = h.Store.UpdateTaskStatus(c.Request.Context(), taskID, store.TaskFailed, 0, "", "入队失败", nowPtr(now))
    util.JSON(c, http.StatusInternalServerError, 9000, "任务入队失败", nil)
    return
  }
